...
if ( ! function_exists( 'gute_about_setup' ) ) :
	...
	function gute_about_setup() {
						$theme = wp_get_theme();
$gute_name = $theme->get( 'Name' );
$theme_slug = $theme->get( 'TextDomain' );


	$config = array(
		// Menu name under Appearance.
		'menu_name'               => sprintf( esc_html__( '%s Info', 'gute' ),$gute_name),
		// Page title.
		'page_name'               => sprintf( esc_html__( '%s Info', 'gute' ),$gute_name),
		...
		// Main welcome content
			...
			'quick_links' => array(
                'gute_pro' => array(
                    'text'   => esc_html__( 'UPGRADE GUTE PRO', 'gute' ),
                    'url'    => 'https://wpthemespace.com/product/gute/?add-to-cart=559',
                    'button' => 'danger',
                ),
                'gute_portfolio' => array(
                    'text'   => esc_html__( 'UPGRADE GUTE PORTFOLIO PRO', 'gute' ),
                    'url'    => 'https://wpthemespace.com/product/gute-portfolio/',
                    'button' => 'danger',
                ),
                'documentation_url' => array(
                    'text'   => esc_html__( 'GUTE PORTFOLIO DEMO', 'gute' ),
                    'url'    => 'http://gp.wpthemium.com',
                    'button' => 'primary',
                ),
                'live_url' => array(
                    'text'   => esc_html__( 'GUTE PRO DEMO', 'gute' ),
			...
			'getting_started' => array(
				'one' =>array(
					...
					'button_text' => esc_html__( 'UPGRADE For  Demo Content', 'gute' ),
					...
					'recommended_actions'        => array(
						...
						'content'            => array(
							'magical-blocks' => array(
								'title'       => __('Magical Blocks', 'gute' ),
								'description' => __( 'Supercharge your Gutenberg block with highly customizable Magical Blocks For Gutenberg.', 'gute' ),
								'plugin_slug' => 'magical-blocks',
								'id' => 'magical-blocks'
							),
							'go-pro' => array(
								'title'       => '<a target="_blank" class="activate-now button button-danger" href="https://wpthemespace.com/product/gute/?add-to-cart=559">'.__('UPGRADE TO PRO','gute').'</a>',
			...
			// Free vs pro array.
			'free_pro'                => array(
				'free_theme_name'     => $gute_name,
				'pro_theme_name'      => $gute_name.__(' Pro','gute'),
...
function gute_admin_notice__error() {
	...
	$gute_name = $gute_theme->get( 'Name' );
	$gute_domain = $gute_theme->get( 'TextDomain' );

	
	
    if(get_option('gotinuer')){
        return;
    }
	$class = 'gute-notice notice notice-warning is-dismissible';
	$message = __( '<strong><span> Hi Buddy!! <span>You are using the free version.</span>  <br> If you want a beautiful, smart, SEO friendly, more secure and unlimited options website then need to upgrade pro. UPGRADE FOR LIFETIME WITH NOMINAL PRICE.</span> <span> PRO License Now Lifetime For You. Buy One Time And Get Lifetime Update, High-Quality performance And support. So Why You late? Build Your Site With Pro Version.</span> ', 'gute' );

if( $gute_domain == 'gute-portfolio' ){
	$url1 = esc_url('https://wpthemespace.com/product/gute-portfolio/?add-to-cart=750');
    $url2 = esc_url('https://wpthemespace.com/product/gute-portfolio/');
}elseif($gute_domain == 'gute'){
	$url1 = esc_url('https://wpthemespace.com/product/gute/?add-to-cart=559');
    $url2 = esc_url('https://wpthemespace.com/product/gute/');
}else{
	$url1 = esc_url('https://wpthemespace.com/product/gute-plus/?add-to-cart=2041');
    $url2 = esc_url('https://wpthemespace.com/product/gute-plus/');
}

    


    $url3 =esc_url('https://www.youtube.com/watch?v=FogqFZy3xfs&t');

	printf( '<div class="%1$s" style="padding:10px 15px 20px;"><p>%2$s</p><a target="_blank" class="button button-primary" href="%3$s" style="margin-right:10px">'.__('Upgrade Pro','gute').'</a><a target="_blank" class="button button-primary" href="%4$s" style="margin-right:10px">'.__('Show details','gute').'</a><a target="_blank" class="button button-primary" href="%5$s" style="margin-right:10px">'.__('Show Video','gute').'</a><button style="margin-left:10px" class="button button-info gute-dismiss">or hide the notice</button></div>', esc_attr( $class ), wp_kses_post( $message ),$url1,$url2,$url3 ); 
}
...
function gute_admin_notice_option(){
    if(isset($_GET['dismissed']) && $_GET['dismissed'] == 1 ){
        delete_option( 'gnotice54h');
        update_option( 'gotinuer', 1 );
    }
}
