...
if ( ! class_exists( 'gute_About' ) ) {
	...
	class gute_About {
		 ...
		 *
		 * @var gute_About $instance The  TI_About_Page instance.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof gute_About ) ) {
				self::$instance = new gute_About;
		...
		public function ti_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap gute-about-wrap">';
				...
				echo '<a href="'.esc_url('https://wpthemespace.com/product/gute-pro/').'" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
    ...
    return add_query_arg(
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section gute-section gute-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .gute-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section gute-section gute-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .gute-section -->
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="'.esc_url('https://wpthemespace.com/product/gute-pro/').'" target="_blank" >';
					echo esc_html ('gute Pro Live Preview', 'gute');
					echo '</a></td></tr>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'about-page-css', get_template_directory_uri() . '/inc/about/css/about.min.css',array(),'1.2.0','all' );
			wp_enqueue_script( 'gute-notice-js', get_template_directory_uri() . '/inc/about/js/notice.js', array('jquery'), '1.7.0', true );
        ...
        'activating_string'        => __( 'Activating', 'gute' )
        )
    );
