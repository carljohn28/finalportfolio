...
if( in_array($pagenow, array('post-new.php', 'post.php'))) {
	...
	wp_enqueue_style('gallerybox-admin', plugins_url('/assets/css/gallerybox-admin.css', __FILE__), array(), '1.7.6', 'all' );
...
add_action( 'plugins_loaded', 'gbox_image_size' );
    ...
    function gbox_adminpro_link($links) {
        $newlink = sprintf("<a target='_blank' href='%s'><span style='color:red;font-weight:bold'>%s</span></a>",esc_url('https://wpthemespace.com/product/gallery-box-pro/?add-to-cart=688'),__('Go Pro','gbox-pro'));
        $links[] = $newlink;
        return $links;
    }
...
function gbox_ewidget_init() {

        // Include Widget files
  require_once( GALLERY_BOX_PATH. '/includes/elementor-addon.php');

        \Elementor\Plugin::instance()->widgets_manager->register_widget_type( new \gBoxEWidget() ); 
        
}
add_action( 'elementor/widgets/widgets_registered', 'gbox_ewidget_init' );
}
 ...
 * @return void
 */
function appsero_init_tracker_gallery_box() {
...
appsero_init_tracker_gallery_box();